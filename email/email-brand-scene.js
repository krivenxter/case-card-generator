...
export function renderBrandSceneMarkup(block, { preview = true, editable = false } = {}) {
  ...
  return `<div data-brand-scene style="position:relative;width:${BRAND_SCENE_WIDTH}px;min-height:${BRAND_SCENE_MIN_HEIGHT}px;box-sizing:border-box;overflow:hidden;padding:34px;border-radius:30px;background:${outerBackground};color:#fff;">
    <div${headingAttrs} style="position:relative;z-index:4;max-width:540px;font-family:'Dela Gothic One','Arial Black',Arial,sans-serif;font-size:${DELA_FONT_SIZES.large}px;line-height:1.2;font-weight:400;text-transform:uppercase;letter-spacing:.02em;">${escapeHtml(content.heading || "ЗАГОЛОВОК ФИРМЕННОГО БЛОКА").replace(/\r?\n/g, "<br>")}</div>
