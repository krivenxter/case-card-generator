...
export const BRAND_TITLE_HEIGHT = 180;
...
export function renderBrandTitleMarkup(block, { editable = false } = {}) {
  ...
  return `<div data-brand-title style="display:flex;width:${BRAND_TITLE_WIDTH}px;height:${BRAND_TITLE_HEIGHT}px;padding:28px 32px;align-items:center;box-sizing:border-box;overflow:hidden;border-radius:28px;background:${background};color:${text};">
    <div${headingAttrs} style="width:100%;font-family:'Dela Gothic One','Arial Black',Arial,sans-serif;font-size:${fontSize}px;line-height:1.12;font-weight:400;text-transform:uppercase;letter-spacing:-.02em;word-break:break-word;">${escapeHtml(content.heading || "ЗАГОЛОВОК DELA")}</div>
  </div>`;
}
